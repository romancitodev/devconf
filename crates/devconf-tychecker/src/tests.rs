...
use crate::{Context, Type, TypeChecker};
...
fn test_literal_types() {
    ...
    assert_eq!(TypeChecker::infer_type(&string_expr).unwrap(), Type::String);

    let unquoted_string = AstExpr::Literal(Literal::UnquotedString("world".to_string()));
    assert_eq!(
        TypeChecker::infer_type(&unquoted_string).unwrap(),
        Type::String
    );

    // Numeric literals
    let int_expr = AstExpr::Literal(Literal::Integer(42));
    assert_eq!(TypeChecker::infer_type(&int_expr).unwrap(), Type::Int);

    let float_expr = AstExpr::Literal(Literal::Float(300.12));
    assert_eq!(TypeChecker::infer_type(&float_expr).unwrap(), Type::Float);

    // Boolean literal
    let bool_expr = AstExpr::Literal(Literal::Boolean(true));
    assert_eq!(TypeChecker::infer_type(&bool_expr).unwrap(), Type::Bool);

    // Null literal
    let null_expr = AstExpr::Literal(Literal::Null);
    assert_eq!(TypeChecker::infer_type(&null_expr).unwrap(), Type::Null);
}

#[test]
fn test_identifier_type() {
    let ident_expr = AstExpr::Ident("my_var".to_string());
    assert_eq!(TypeChecker::infer_type(&ident_expr).unwrap(), Type::String);
}
...
fn test_binary_operations() {
    ...
    assert_eq!(TypeChecker::infer_type(&and_expr).unwrap(), Type::String);
    ...
    let result = TypeChecker::infer_type(&or_expr).unwrap();
    ...
    assert_eq!(TypeChecker::infer_type(&eq_expr).unwrap(), Type::Bool);
    ...
    assert_eq!(TypeChecker::infer_type(&ne_expr).unwrap(), Type::Bool);
}
...
fn test_unary_operations() {
    ...
    assert_eq!(TypeChecker::infer_type(&not_expr).unwrap(), Type::Bool);
}
...
fn test_array_types() {
    ...
    assert_eq!(
        TypeChecker::infer_type(&empty_array).unwrap(),
    ...
    assert_eq!(
        TypeChecker::infer_type(&string_array).unwrap(),
    ...
    let result = TypeChecker::infer_type(&mixed_array).unwrap();
...
fn test_object_types() {
    ...
    let result = TypeChecker::infer_type(&object_expr).unwrap();
...
fn test_cast_expressions() {
    ...
    assert_eq!(TypeChecker::infer_type(&string_to_int).unwrap(), Type::Int);

    let int_to_float = AstExpr::Cast {
        expr: Box::new(AstExpr::Literal(Literal::Integer(42))),
        ty: "float".to_string(),
    };
    assert_eq!(TypeChecker::infer_type(&int_to_float).unwrap(), Type::Float);
    ...
    assert!(TypeChecker::infer_type(&bool_to_int).is_err());
}
...
fn test_interpolation_expressions() {
    ...
    assert_eq!(
        TypeChecker::infer_type(&simple_interp).unwrap(),
        Type::String
    );
    ...
    assert_eq!(TypeChecker::infer_type(&cast_interp).unwrap(), Type::Int);
    ...
    assert!(TypeChecker::infer_type(&invalid_cast_interp).is_err());
}
...
fn test_string_to_type_conversion() {
    assert_eq!(TypeChecker::string_to_type("str"), Type::String);
    assert_eq!(TypeChecker::string_to_type("int"), Type::Int);
    assert_eq!(TypeChecker::string_to_type("float"), Type::Float);
    assert_eq!(TypeChecker::string_to_type("bool"), Type::Bool);
    // Unknown type should return error
    assert_eq!(TypeChecker::string_to_type("unknown_type"), Type::Unknown);
}

#[test]
fn test_cast_resolution() {
    // String can cast to anything
    assert!(TypeChecker::resolve_cast(&Type::String, &Type::Int));
    assert!(TypeChecker::resolve_cast(&Type::String, &Type::Float));
    assert!(TypeChecker::resolve_cast(&Type::String, &Type::Bool));

    // Numbers can cast between each other
    assert!(TypeChecker::resolve_cast(&Type::Int, &Type::Float));
    assert!(TypeChecker::resolve_cast(&Type::Float, &Type::Int));

    // Same types can "cast" to themselves
    assert!(TypeChecker::resolve_cast(&Type::String, &Type::String));
    assert!(TypeChecker::resolve_cast(&Type::Int, &Type::Int));

    // Invalid casts
    assert!(!TypeChecker::resolve_cast(&Type::Bool, &Type::Int));
    assert!(!TypeChecker::resolve_cast(
        &Type::Array(Box::new(Type::String)),
        &Type::String
    ));
}
...
fn test_complex_nested_expressions() {
    ...
    let result = TypeChecker::infer_type(&complex_expr).unwrap();
    ...
    let result = TypeChecker::infer_type(&array_of_objects).unwrap();
    if let Type::Array(inner) = result {
...
fn test_error_messages() {
    ...
    let error = TypeChecker::infer_type(&invalid_cast).unwrap_err();
    ...
    let t = TypeChecker::string_to_type("invalid_type");
